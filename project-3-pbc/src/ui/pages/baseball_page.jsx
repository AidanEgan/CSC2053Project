...
import {Button, Grid, Table, TableBody, TableRow, TableCell, CircularProgress, TableHead, Dialog, DialogTitle, DialogContent, DialogContentText, TextField, Typography, DialogActions} from "@material-ui/core";
...
export default class BaseballPage extends ListenerComponent  {
    ...
    constructor (props) {
        ...
        this.renderPitcherTableBody = this.renderPitcherTableBody.bind(this); 
        this.renderHitterTableBody = this.renderHitterTableBody.bind(this);
    ...
    renderHitterTableBody () {
        if (!HitterStore.instance().isLoaded()) {
            return (
                <CircularProgress/>
            );
        }
        let players = HitterStore.instance().getPlayers(); 
        console.log(players);
    ...
    renderPitcherTableBody () {
        if (!PitcherStore.instance().isLoaded()) {
            return (
                <CircularProgress/>
            );
        }
        let players = PitcherStore.instance().getPlayers(); 
        return(players.map((player)=>{
            return (
                <TableRow>
            ...
            <React.Fragment>
                ...
                <DialogActions>
                    <Button variant='outlined' onClick={() => {
                        ...
                        var teamBudget = parseFloat(this.salaryCap) / 2.196362;
